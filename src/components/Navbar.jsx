...
export default function Navbar() {
  const { session, isStaff, isHR, isManagement, conductsInterviews } = useAuth();
    ...
    <header className="sticky top-0 z-40 bg-white/40 backdrop-blur-2xl backdrop-saturate-150 border-b border-ink/10 text-ink shadow-[0_2px_24px_-12px_rgba(0,0,0,0.4)]">
      <div className="max-w-6xl mx-auto px-5 flex items-center justify-between h-16">
        ...
        <nav className="flex items-center gap-1 sm:gap-2 text-sm font-medium">
          {/* Lands on the listings rather than the hero — someone
              clicking "Jobs" wants the jobs, not the banner. */}
          <Link to="/#open-positions" className={linkClass}>
            Jobs
          </Link>
